...
class DailyListFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        mainViewModel.allExtendedDiaries.observe(viewLifecycleOwner, Observer {
            ...
            if(listSize != 0) {
                val i = ObjectAnimator.ofInt(layout.progressBar_appbar, "progress", (listDoneSize * 100) / listSize)
                i.interpolator = FastOutLinearInInterpolator()
                i.duration = 500
                i.addUpdateListener {
                    layout.progressbar_text.text = i.animatedValue.toString() + " " + resources.getString(R.string.progressbar_progress)
                }
                i.start()
            }
            else
    ...
    private fun findDailyListItems(allDiariesList : List<ExtendedDiary>, extDiaryParent : ExtendedDiary?) : List<DailyListItem>{
